...
namespace Lab_3
{
    class Pentagon : figure
    {
        ...
        { 
            return side / 4 * Math.Sqrt(25 + 10 * Math.Sqrt(5)); 
        }
        ...
        public override void Draw(Graphics gr)
        {
            ...
            var points = Enumerable.Range(0, Vertexes).Select(i => PointF.Add(GetCenter(),
                new SizeF((float)Math.Sin(angle * i) * (float)radius, (float)Math.Cos(angle * i) * (float)radius)));
